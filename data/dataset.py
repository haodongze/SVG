...
class VisDialDataset(Dataset):
  ...
  def __init__(
    ...
    if sample_flag == True:
      samplefile = open("/data1/dzhao/rva/data/visdial_1.0_train_dense_sample.json", 'r')  ####for answer score sampling (fine-tune)
      sample = json.loads(samplefile.read())
      samplefile.close()
      ndcg_id_list = []
      for idx in range(len(sample)):
        ndcg_id_list.append(sample[idx]['image_id'])
      self.text_feat_image_ids = ndcg_id_list

      self.id2index = {}
      for i, img_id in enumerate(list(self.hdf_reader.text_feature_id_l)):
        self.id2index[img_id] = i
    print("image ids", len(self.text_feat_image_ids))
    if overfit:
      self.text_feat_image_ids = self.text_feat_image_ids[:5]
    self.float_variables = ["img_feat", "gt_relevance", "img_boxes"]
  ...
  def __getitem__(self, index):
    ...
    if self.sample_flag == True:
      index = self.id2index[self.text_feat_image_ids[index]]
...
class DataHdfReader(object):
  ...
  def __init__(self, hparams, text_features_h5_path: str, img_features_h5_path: str, split=None, old_split=None):
    ...
    print(self.text_features_h5_path)
  ...
  def __getitem__(self, index: int):
    ...
    if self.hparams.img_feature_type == "dan_faster_rcnn_x101":
      with h5py.File(self.img_features_h5_path, "r") as features_hdf:
        ...
        bounding_boxes = features_hdf["image_bb"][self.pos_boxes[img_feature_index][0]: self.pos_boxes[img_feature_index][1], :]
        ...
        features["img_boxes"] = bounding_boxes
    else:
